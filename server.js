...
// Conexión con MongoDB Atlas / Compass.
// Aquí pega el string de conexión de Atlas. Al final del enlace, coloca el nombre de la base de datos.
// Ejemplo: mongodb+srv://usuario:password@cluster.mongodb.net/SmartSipDB?retryWrites=true&w=majority&appName=myapp
...
app.get('/api/consumption/weekly', verifyToken, async (req, res) => {
    try {
        const todayConsumption = await getOrCreateTodayConsumption(req.user.userId);
        const todayLiters = roundTo2(todayConsumption.consumedLiters);

        return res.json({
            bars: [
                { label: 'Lun', liters: 1.7 },
                { label: 'Mar', liters: 2.1 },
                { label: 'Mié', liters: 1.4 },
                { label: 'Jue', liters: 2.6 },
                { label: 'Vie', liters: 1.9 },
                { label: 'Dom', liters: 2.8 },
                { label: 'Hoy', liters: todayLiters }
            ]
        });
    } catch (error) {
...
app.listen(port, () => {
    console.log('Aplicación SmartSip corriendo en puerto ' + port);
});
